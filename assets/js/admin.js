...
(function() {
    ...
    const OrbitoolsAdmin = {
            ...
            window.fetch = function(...args) {
                return originalFetch.apply(this, args)
                    .then(function(response) {
                        ...
                        if (args[0] === orbitoolsAdminKit.ajaxUrl && args[1] && args[1].body) {
                            const formData = args[1].body;

                            // Check if this is a settings save action
                            if (formData.get && formData.get('action') === 'orbitools_adminkit_save_settings_orbitools') {
        ...
        reloadPage: function(message) {
            ...
            if (window.OrbitoolsAdminKit) {
                window.OrbitoolsAdminKit.showNotice(message, 'info');
            }
    ...
    function initOrbitoolsAdmin() {
        ...
        if (document.querySelector('.orbi-admin') && window.orbitoolsAdminKit && orbitoolsAdminKit.slug === 'orbitools') {
            OrbitoolsAdmin.init();
        }
    }
